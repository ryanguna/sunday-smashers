...
function demoRows(): TeamsAdminRows {
  return {
...
function emptyRows(): TeamsAdminRows {
  return { divisions: [], teams: [], freeAgents: [] }
}

/**
 * A configured project with nobody registered yet returns empty lists, not the
 * demo bench — see `@/lib/demo-mode` for the one rule.
 */
export const getTeamsAdminData = cache(async function getTeamsAdminData(): Promise<TeamsAdminData> {
  const { data, isDemo, error } = await loadLiveOrDemo<TeamsAdminRows>({
    demo: demoRows,
    empty: emptyRows,
    live: loadLive,
  })
...
async function loadLive(): Promise<TeamsAdminRows> {
  ...
  const teamList = rowsOrThrow(teamResult) as TeamRow[]
  const memberList = rowsOrThrow(teamMemberResult) as TeamMemberRow[]
